---
argument-hint: "[--commit] or leave empty to commit and push"
...
4. **Push to Origin** (unless --commit specified): Push committed changes to the current origin branch

## Usage Examples

- `/git:push-updates` - Commit with sign-off and push to origin (default)
- `/git:push-updates --commit` - Commit with sign-off only, skip push
...
- Push to origin by default unless `--commit` parameter is provided
...
- Verifies commit success before push (unless disabled with --commit)
